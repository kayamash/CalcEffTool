...
execute="./run.sh"
for filenumber in {00..29};do
    sed -i -e "s/div00/div${filenumber}/g" run.sh
    sed -i -e "s/div00/div${filenumber}/g" CalcEffAlg_options.py
    eval $execute
    sleep 10s
    sed -i -e "s/div${filenumber}/div00/g" run.sh
    sed -i -e "s/div${filenumber}/div00/g" CalcEffAlg_options.py
done
