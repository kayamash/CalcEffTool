...
int TagAndProbeFCBM::execute(const xAOD::MuonContainer* muons, const xAOD::MuonRoIContainer* rois){
    ...
    for(auto mu_itr = mu_begin; mu_itr != mu_end; ++mu_itr ){
        ...
        uint32_t SAroiNumber, SAroiSector;
...
int TagAndProbeFCBM::matchEFFS( const string& chain, const xAOD::Muon* muon, const Trig::FeatureContainer& fc, double* reqdR, double* param, double* tagparam ) {
  ...
  for ( auto& fEF : fEFs ) {
    ...
    for( const auto& ef : *cont ) {
      ...
      efCharge = ef->charge();
      efdRmu   = m_utils.deltaR( muEta, muPhi, efEta, efPhi );
      //cout << "EFFS: " << efPt << ":" << efEta << ":" << efPhi << ":" << efdRmu << endl;
    }
  }
  bool pass = ( m_utils.muonEFThreshold( efEta, efPt*0.001, chain ) );
  //if( ( chain=="HLT_mu20" || chain=="HLT_mu20_msonly" )&& pass ) cout << "EF : " << muPt << ", " << efPt << " ==> " << chain << ": " << pass << endl;
  if( m_message>1 ) cout << "EF : " << muPt << ", " << efPt << " ==> " << chain << ": " << pass << endl;
  if( pass ) {
    isPassedEF = 1;
    *reqdR    = efdRmu;
...
bool TagAndProbeFCBM::CBHypo( double pt, int EtaRegion)
{
    ...
    if( pt != -99999. && fabs( pt * 0.001 ) > threshold ) pass = kTRUE;
    return pass;
}
