...
int EventTreeForCBM::filltree( TagAndProbe& tap, unsigned long long int eventNumber, int runNumber, int lumiBlock, float averageInteractionsPerCrossing) {
    ...
    vector < double > tmp_muon_offline_extinneta                        = tap.muonOfflineExtInnEta();
    muon_offline_extinneta                                              = &( tmp_muon_offline_extinneta );
    vector < double > tmp_muon_offline_phi                              = tap.muonOfflinePhi();
    muon_offline_phi                                                    = &( tmp_muon_offline_phi );
    vector < double > tmp_muon_offline_extphi                           = tap.muonOfflineExtPhi();
    muon_offline_extphi                                                 = &( tmp_muon_offline_extphi );
    vector < double > tmp_muon_offline_extinnphi                        = tap.muonOfflineExtInnPhi();
    muon_offline_extinnphi                                              = &( tmp_muon_offline_extinnphi );
    vector < double > tmp_muon_offline_d0                               = tap.muonOfflineD0();
    muon_offline_d0                                                     = &( tmp_muon_offline_d0 );
    vector < double > tmp_muon_offline_z0                               = tap.muonOfflineZ0();
    muon_offline_z0                                                     = &( tmp_muon_offline_z0 );
    vector < double > tmp_muon_offline_charge                           = tap.muonOfflineCharge();
    muon_offline_charge                                                 = &( tmp_muon_offline_charge );
    vector < int > tmp_muon_n_offlineSegment                            = tap.muonOfflineSegmentN();
    muon_n_offlineSegment                                               = &( tmp_muon_n_offlineSegment );
    vector < vector < double > > tmp_muon_offlineSegment_x              = tap.muonOfflineSegmentX();
    muon_offlineSegment_x                                               = &( tmp_muon_offlineSegment_x );
    vector < vector < double > > tmp_muon_offlineSegment_y              = tap.muonOfflineSegmentY();
    muon_offlineSegment_y                                               = &( tmp_muon_offlineSegment_y );
    vector < vector < double > > tmp_muon_offlineSegment_z              = tap.muonOfflineSegmentZ();
    muon_offlineSegment_z                                               = &( tmp_muon_offlineSegment_z );
    vector < vector < double > > tmp_muon_offlineSegment_px             = tap.muonOfflineSegmentPx();
    muon_offlineSegment_px                                              = &( tmp_muon_offlineSegment_px );
    vector < vector < double > > tmp_muon_offlineSegment_py             = tap.muonOfflineSegmentPy();
    muon_offlineSegment_py                                              = &( tmp_muon_offlineSegment_py );
    vector < vector < double > > tmp_muon_offlineSegment_pz             = tap.muonOfflineSegmentPz();
    muon_offlineSegment_pz                                              = &( tmp_muon_offlineSegment_pz );
    vector < vector < double > > tmp_muon_offlineSegment_chiSquared     = tap.muonOfflineSegmentChiSquared();
    muon_offlineSegment_chiSquared                                      = &( tmp_muon_offlineSegment_chiSquared );
    vector < vector < double > > tmp_muon_offlineSegment_NumberDoF      = tap.muonOfflineSegmentNumberDoF();
    muon_offlineSegment_NumberDoF                                       = &( tmp_muon_offlineSegment_NumberDoF );
    vector < vector < double > > tmp_muon_offlineSegment_sector         = tap.muonOfflineSegmentSector();
    muon_offlineSegment_sector                                          = &( tmp_muon_offlineSegment_sector );
    vector < vector < double > > tmp_muon_offlineSegment_chamberIndex   = tap.muonOfflineSegmentChamberIndex();
    muon_offlineSegment_chamberIndex                                    = &( tmp_muon_offlineSegment_chamberIndex );
    vector < vector < double > > tmp_muon_offlineSegment_etaIndex       = tap.muonOfflineSegmentEtaIndex();
    muon_offlineSegment_etaIndex                                        = &( tmp_muon_offlineSegment_etaIndex );
    vector < vector < double > > tmp_muon_offlineSegment_nPrecisionHits = tap.muonOfflineSegmentNPrecisionHits();
    muon_offlineSegment_nPrecisionHits                                  = &( tmp_muon_offlineSegment_nPrecisionHits );
    vector < vector < double > > tmp_muon_offlineSegment_NPhiLayers     = tap.muonOfflineSegmentNPhiLayers();
    muon_offlineSegment_NPhiLayers                                      = &( tmp_muon_offlineSegment_NPhiLayers );
    vector < vector < double > > tmp_muon_offlineSegment_NTrigEtaLayers = tap.muonOfflineSegmentNTrigEtaLayers();
    muon_offlineSegment_NTrigEtaLayers                                  = &( tmp_muon_offlineSegment_NTrigEtaLayers );
    vector < int > tmp_muon_EFTAG_pass                                  = tap.muonEFTAGPass();
    muon_EFTAG_pass                                                     = &( tmp_muon_EFTAG_pass );
    vector < double > tmp_muon_EFTAG_pt                                 = tap.muonEFTAGPt();
    muon_EFTAG_pt                                                       = &( tmp_muon_EFTAG_pt );
    vector < double > tmp_muon_EFTAG_eta                                = tap.muonEFTAGEta();
    muon_EFTAG_eta                                                      = &( tmp_muon_EFTAG_eta );
    vector < double > tmp_muon_EFTAG_phi                                = tap.muonEFTAGPhi();
    muon_EFTAG_phi                                                      = &( tmp_muon_EFTAG_phi );
    vector < int > tmp_muon_L1_pass                                     = tap.muonL1Pass();
    muon_L1_pass                                                        = &( tmp_muon_L1_pass );
    vector <double > tmp_muon_L1_pt                                     = tap.muonL1Pt();
    muon_L1_pt                                                          = & ( tmp_muon_L1_pt );
    vector < double > tmp_muon_L1_eta                                   = tap.muonL1Eta();
    muon_L1_eta                                                         = &( tmp_muon_L1_eta );
    vector < double > tmp_muon_L1_phi                                   = tap.muonL1Phi();
    muon_L1_phi                                                         = &( tmp_muon_L1_phi );
    vector < int > tmp_muon_L1_RoINumber                                = tap.muonL1roiNumber();
    muon_L1_RoINumber                                                   = &( tmp_muon_L1_RoINumber );
    vector < int > tmp_muon_L1_RoISector                                = tap.muonL1roiSector();
    muon_L1_RoISector                                                   = &( tmp_muon_L1_RoISector );
    vector < int > tmp_muon_SA_HypoPass                                 = tap.muonSAHypoPass();
    muon_SA_HypoPass                                                    = &( tmp_muon_SA_HypoPass );
    vector < bool > tmp_muon_SA_OvRmPass                                = tap.muonSAOvRmPass();
    muon_SA_OvRmPass                                                    = &( tmp_muon_SA_OvRmPass );
    vector < double > tmp_muon_SA_pt                                    = tap.muonSAPt();
    muon_SA_pt                                                          = &( tmp_muon_SA_pt );
    vector < double > tmp_muon_SA_eta                                   = tap.muonSAEta();
    muon_SA_eta                                                         = &( tmp_muon_SA_eta );
    vector < double > tmp_muon_SA_phi                                   = tap.muonSAPhi();
    muon_SA_phi                                                         = &( tmp_muon_SA_phi );
    vector < double > tmp_muon_SA_etaMS                                 = tap.muonSAEtaMS();
    muon_SA_etaMS                                                       = &( tmp_muon_SA_etaMS );
    vector < double > tmp_muon_SA_phiMS                                 = tap.muonSAPhiMS();
    muon_SA_phiMS                                                       = &( tmp_muon_SA_phiMS );
    vector < double > tmp_muon_SA_etaBE                                 = tap.muonSAEtaBE();
    muon_SA_etaBE                                                       = &( tmp_muon_SA_etaBE );
    vector < double > tmp_muon_SA_phiBE                                 = tap.muonSAPhiBE();
    muon_SA_phiBE                                                       = &( tmp_muon_SA_phiBE );
    vector < double > tmp_muon_SA_TGCpt                                 = tap.muonSAtgcPt();
    muon_SA_TGCpt                                                       = &( tmp_muon_SA_TGCpt );
    vector < double > tmp_muon_SA_ptBarrelRadius                        = tap.muonSAPtBarrelRadius();
    muon_SA_ptBarrelRadius                                              = &( tmp_muon_SA_ptBarrelRadius );
    vector < double > tmp_muon_SA_ptBarrelSagitta                       = tap.muonSAPtBarrelSagitta();
    muon_SA_ptBarrelSagitta                                             = &( tmp_muon_SA_ptBarrelSagitta );
    vector < double > tmp_muon_SA_ptEndcapAlpha                         = tap.muonSAPtEndcapAlpha();
    muon_SA_ptEndcapAlpha                                               = &( tmp_muon_SA_ptEndcapAlpha );
    vector < double > tmp_muon_SA_ptEndcapBeta                          = tap.muonSAPtEndcapBeta();
    muon_SA_ptEndcapBeta                                                = &( tmp_muon_SA_ptEndcapBeta );
    vector < double > tmp_muon_SA_ptEndcapRadius                        = tap.muonSAPtEndcapRadius();
    muon_SA_ptEndcapRadius                                              = &( tmp_muon_SA_ptEndcapRadius );
    vector < double > tmp_muon_SA_CSCpt                                 = tap.muonSAcscPt();
    muon_SA_CSCpt                                                       = &( tmp_muon_SA_CSCpt );
    vector < double > tmp_muon_SA_sAddress                              = tap.muonSAsAddress();
    muon_SA_sAddress                                                    = &( tmp_muon_SA_sAddress );
    vector < double > tmp_muon_SA_SP_rBI                                = tap.muonSASuperPointRBI();
    muon_SA_SP_rBI                                                      = &( tmp_muon_SA_SP_rBI );
    vector < double > tmp_muon_SA_SP_rBM                                = tap.muonSASuperPointRBM();
    muon_SA_SP_rBM                                                      = &( tmp_muon_SA_SP_rBM );
    vector < double > tmp_muon_SA_SP_rBO                                = tap.muonSASuperPointRBO();
    muon_SA_SP_rBO                                                      = &( tmp_muon_SA_SP_rBO );
    vector < double > tmp_muon_SA_SP_rEI                                = tap.muonSASuperPointREI();
    muon_SA_SP_rEI                                                      = &( tmp_muon_SA_SP_rEI );
    vector < double > tmp_muon_SA_SP_rEM                                = tap.muonSASuperPointREM();
    muon_SA_SP_rEM                                                      = &( tmp_muon_SA_SP_rEM);
    vector < double > tmp_muon_SA_SP_rEO                                = tap.muonSASuperPointREO();
    muon_SA_SP_rEO                                                      = &( tmp_muon_SA_SP_rEO );
    vector < double > tmp_muon_SA_SP_rEE                                = tap.muonSASuperPointREE();
    muon_SA_SP_rEE                                                      = &( tmp_muon_SA_SP_rEE );
    vector < double > tmp_muon_SA_SP_rCSC                               = tap.muonSASuperPointRCSC();
    muon_SA_SP_rCSC                                                     = &( tmp_muon_SA_SP_rCSC);
    vector < double > tmp_muon_SA_SP_rBEE                               = tap.muonSASuperPointRBEE();
    muon_SA_SP_rBEE                                                     = &( tmp_muon_SA_SP_rBEE );
    vector < double > tmp_muon_SA_SP_rBME                               = tap.muonSASuperPointRBME();
    muon_SA_SP_rBME                                                     = &( tmp_muon_SA_SP_rBME );
    vector < double > tmp_muon_SA_SP_zBI                                = tap.muonSASuperPointZBI();
    muon_SA_SP_zBI                                                      = &( tmp_muon_SA_SP_zBI );
    vector < double > tmp_muon_SA_SP_zBM                                = tap.muonSASuperPointZBM();
    muon_SA_SP_zBM                                                      = &( tmp_muon_SA_SP_zBM );
    vector < double > tmp_muon_SA_SP_zBO                                = tap.muonSASuperPointZBO();
    muon_SA_SP_zBO                                                      = &( tmp_muon_SA_SP_zBO );
    vector < double > tmp_muon_SA_SP_zEI                                = tap.muonSASuperPointZEI();
    muon_SA_SP_zEI                                                      = &( tmp_muon_SA_SP_zEI );
    vector < double > tmp_muon_SA_SP_zEM                                = tap.muonSASuperPointZEM();
    muon_SA_SP_zEM                                                      = &( tmp_muon_SA_SP_zEM);
    vector < double > tmp_muon_SA_SP_zEO                                = tap.muonSASuperPointZEO();
    muon_SA_SP_zEO                                                      = &( tmp_muon_SA_SP_zEO );
    vector < double > tmp_muon_SA_SP_zEE                                = tap.muonSASuperPointZEE();
    muon_SA_SP_zEE                                                      = &( tmp_muon_SA_SP_zEE );
    vector < double > tmp_muon_SA_SP_zCSC                               = tap.muonSASuperPointZCSC();
    muon_SA_SP_zCSC                                                     = &( tmp_muon_SA_SP_zCSC);
    vector < double > tmp_muon_SA_SP_zBEE                               = tap.muonSASuperPointZBEE();
    muon_SA_SP_zBEE                                                     = &( tmp_muon_SA_SP_zBEE );
    vector < double > tmp_muon_SA_SP_zBME                               = tap.muonSASuperPointZBME();
    muon_SA_SP_zBME                                                     = &( tmp_muon_SA_SP_zBME );
    vector < double > tmp_muon_SA_SP_slopeBI                            = tap.muonSASuperPointSlopeBI();
    muon_SA_SP_slopeBI                                                  = &( tmp_muon_SA_SP_slopeBI );
    vector < double > tmp_muon_SA_SP_slopeBM                            = tap.muonSASuperPointSlopeBM();
    muon_SA_SP_slopeBM                                                  = &( tmp_muon_SA_SP_slopeBM );
    vector < double > tmp_muon_SA_SP_slopeBO                            = tap.muonSASuperPointSlopeBO();
    muon_SA_SP_slopeBO                                                  = &( tmp_muon_SA_SP_slopeBO );
    vector < double > tmp_muon_SA_SP_slopeEI                            = tap.muonSASuperPointSlopeEI();
    muon_SA_SP_slopeEI                                                  = &( tmp_muon_SA_SP_slopeEI );
    vector < double > tmp_muon_SA_SP_slopeEM                            = tap.muonSASuperPointSlopeEM();
    muon_SA_SP_slopeEM                                                  = &( tmp_muon_SA_SP_slopeEM);
    vector < double > tmp_muon_SA_SP_slopeEO                            = tap.muonSASuperPointSlopeEO();
    muon_SA_SP_slopeEO                                                  = &( tmp_muon_SA_SP_slopeEO );
    vector < double > tmp_muon_SA_SP_slopeEE                            = tap.muonSASuperPointSlopeEE();
    muon_SA_SP_slopeEE                                                  = &( tmp_muon_SA_SP_slopeEE );
    vector < double > tmp_muon_SA_SP_slopeCSC                           = tap.muonSASuperPointSlopeCSC();
    muon_SA_SP_slopeCSC                                                 = &( tmp_muon_SA_SP_slopeCSC);
    vector < double > tmp_muon_SA_SP_slopeBEE                           = tap.muonSASuperPointSlopeBEE();
    muon_SA_SP_slopeBEE                                                 = &( tmp_muon_SA_SP_slopeBEE );
    vector < double > tmp_muon_SA_SP_slopeBME                           = tap.muonSASuperPointSlopeBME();
    muon_SA_SP_slopeBME                                                 = &( tmp_muon_SA_SP_slopeBME );
    vector < double > tmp_muon_SA_SP_interceptBI                        = tap.muonSASuperPointInterceptBI();
    muon_SA_SP_interceptBI                                              = &( tmp_muon_SA_SP_interceptBI );
    vector < double > tmp_muon_SA_SP_interceptBM                        = tap.muonSASuperPointInterceptBM();
    muon_SA_SP_interceptBM                                              = &( tmp_muon_SA_SP_interceptBM );
    vector < double > tmp_muon_SA_SP_interceptBO                        = tap.muonSASuperPointInterceptBO();
    muon_SA_SP_interceptBO                                              = &( tmp_muon_SA_SP_interceptBO );
    vector < double > tmp_muon_SA_SP_interceptEI                        = tap.muonSASuperPointInterceptEI();
    muon_SA_SP_interceptEI                                              = &( tmp_muon_SA_SP_interceptEI );
    vector < double > tmp_muon_SA_SP_interceptEM                        = tap.muonSASuperPointInterceptEM();
    muon_SA_SP_interceptEM                                              = &( tmp_muon_SA_SP_interceptEM);
    vector < double > tmp_muon_SA_SP_interceptEO                        = tap.muonSASuperPointInterceptEO();
    muon_SA_SP_interceptEO                                              = &( tmp_muon_SA_SP_interceptEO );
    vector < double > tmp_muon_SA_SP_interceptEE                        = tap.muonSASuperPointInterceptEE();
    muon_SA_SP_interceptEE                                              = &( tmp_muon_SA_SP_interceptEE );
    vector < double > tmp_muon_SA_SP_interceptCSC                       = tap.muonSASuperPointInterceptCSC();
    muon_SA_SP_interceptCSC                                             = &( tmp_muon_SA_SP_interceptCSC);
    vector < double > tmp_muon_SA_SP_interceptBEE                       = tap.muonSASuperPointInterceptBEE();
    muon_SA_SP_interceptBEE                                             = &( tmp_muon_SA_SP_interceptBEE );
    vector < double > tmp_muon_SA_SP_interceptBME                       = tap.muonSASuperPointInterceptBME();
    muon_SA_SP_interceptBME                                             = &( tmp_muon_SA_SP_interceptBME );
    vector < double > tmp_muon_SA_SP_chi2BI                             = tap.muonSASuperPointChi2BI();
    muon_SA_SP_chi2BI                                                   = &( tmp_muon_SA_SP_chi2BI );
    vector < double > tmp_muon_SA_SP_chi2BM                             = tap.muonSASuperPointChi2BM();
    muon_SA_SP_chi2BM                                                   = &( tmp_muon_SA_SP_chi2BM );
    vector < double > tmp_muon_SA_SP_chi2BO                             = tap.muonSASuperPointChi2BO();
    muon_SA_SP_chi2BO                                                   = &( tmp_muon_SA_SP_chi2BO );
    vector < double > tmp_muon_SA_SP_chi2EI                             = tap.muonSASuperPointChi2EI();
    muon_SA_SP_chi2EI                                                   = &( tmp_muon_SA_SP_chi2EI );
    vector < double > tmp_muon_SA_SP_chi2EM                             = tap.muonSASuperPointChi2EM();
    muon_SA_SP_chi2EM                                                   = &( tmp_muon_SA_SP_chi2EM);
    vector < double > tmp_muon_SA_SP_chi2EO                             = tap.muonSASuperPointChi2EO();
    muon_SA_SP_chi2EO                                                   = &( tmp_muon_SA_SP_chi2EO );
    vector < double > tmp_muon_SA_SP_chi2EE                             = tap.muonSASuperPointChi2EE();
    muon_SA_SP_chi2EE                                                   = &( tmp_muon_SA_SP_chi2EE );
    vector < double > tmp_muon_SA_SP_chi2CSC                            = tap.muonSASuperPointChi2CSC();
    muon_SA_SP_chi2CSC                                                  = &( tmp_muon_SA_SP_chi2CSC);
    vector < double > tmp_muon_SA_SP_chi2BEE                            = tap.muonSASuperPointChi2BEE();
    muon_SA_SP_chi2BEE                                                  = &( tmp_muon_SA_SP_chi2BEE );
    vector < double > tmp_muon_SA_SP_chi2BME                            = tap.muonSASuperPointChi2BME();
    muon_SA_SP_chi2BME                                                  = &( tmp_muon_SA_SP_chi2BME );
    vector < float > tmp_muon_SA_RoIEta                                 = tap.muonSAroiEta();
    muon_SA_RoIEta                                                      = &( tmp_muon_SA_RoIEta );
    vector < float > tmp_muon_SA_RoIPhi                                 = tap.muonSAroiPhi();
    muon_SA_RoIPhi                                                      = &( tmp_muon_SA_RoIPhi );
    vector < double > tmp_muon_SA_isRPCFailure                          = tap.muonSAisrpcFailure();
    muon_SA_isRPCFailure                                                = &( tmp_muon_SA_isRPCFailure );
    vector < double > tmp_muon_SA_isTGCFailure                          = tap.muonSAistgcFailure();
    muon_SA_isTGCFailure                                                = &( tmp_muon_SA_isTGCFailure );
    vector < double > tmp_muon_SA_BarrelRadius                          = tap.muonSABarrelRadius();
    muon_SA_BarrelRadius                                                = &( tmp_muon_SA_BarrelRadius );
    vector < double > tmp_muon_SA_BarrelSagitta                         = tap.muonSABarrelSagitta();
    muon_SA_BarrelSagitta                                               = &( tmp_muon_SA_BarrelSagitta );
    vector < double > tmp_muon_SA_EtaMap                                = tap.muonSAEtaMap();
    muon_SA_EtaMap                                                      = &( tmp_muon_SA_EtaMap );
    vector < double > tmp_muon_SA_PhiMap                                = tap.muonSAPhiMap();
    muon_SA_PhiMap                                                      = &( tmp_muon_SA_PhiMap );
    vector < uint32_t > tmp_muon_SA_RoINumber                           = tap.muonSAroiNumber();
    muon_SA_RoINumber                                                   = &( tmp_muon_SA_RoINumber );
    vector < uint32_t > tmp_muon_SA_RoISector                           = tap.muonSAroiSector();
    muon_SA_RoISector                                                   = &( tmp_muon_SA_RoISector );
    vector < vector < float > > tmp_muon_SA_RPCHit_x                    = tap.muonSArpcHitX();
    muon_SA_RPCHit_x                                                    = &( tmp_muon_SA_RPCHit_x );
    vector < vector < float > > tmp_muon_SA_RPCHit_y                    = tap.muonSArpcHitY();
    muon_SA_RPCHit_y                                                    = &( tmp_muon_SA_RPCHit_y );
    vector < vector < float > > tmp_muon_SA_RPCHit_z                    = tap.muonSArpcHitZ();
    muon_SA_RPCHit_z                                                    = &( tmp_muon_SA_RPCHit_z );
    vector < vector < double > > tmp_muon_SA_RPCHit_r                   = tap.muonSArpcHitR();
    muon_SA_RPCHit_r                                                    = &( tmp_muon_SA_RPCHit_r );
    vector < vector < double > > tmp_muon_SA_RPCHit_eta                 = tap.muonSArpcHitEta();
    muon_SA_RPCHit_eta                                                  = &( tmp_muon_SA_RPCHit_eta );
    vector < vector < double > > tmp_muon_SA_RPCHit_phi                 = tap.muonSArpcHitPhi();
    muon_SA_RPCHit_phi                                                  = &( tmp_muon_SA_RPCHit_phi );
    vector < vector < uint32_t > > tmp_muon_SA_RPCHit_measuresPhi       = tap.muonSArpcHitMeasuresPhi();
    muon_SA_RPCHit_measuresPhi                                          = &( tmp_muon_SA_RPCHit_measuresPhi );
    vector < vector < string > > tmp_muon_SA_RPCHit_stationName         = tap.muonSArpcHitStationName();
    muon_SA_RPCHit_stationName                                          = &( tmp_muon_SA_RPCHit_stationName );
    vector < vector < double > > tmp_muon_SA_RPCHit_stationNumber       = tap.muonSArpcHitStationNumber();
    muon_SA_RPCHit_stationNumber                                        = &( tmp_muon_SA_RPCHit_stationNumber );
    vector < float > tmp_muon_SA_RPCFit_innPhi                          = tap.muonSArpcFitInnPhi();
    muon_SA_RPCFit_innPhi                                               = &( tmp_muon_SA_RPCFit_innPhi );
    vector < float > tmp_muon_SA_RPCFit_innSlope                        = tap.muonSArpcFitInnSlope();
    muon_SA_RPCFit_innSlope                                             = &( tmp_muon_SA_RPCFit_innSlope );
    vector < float > tmp_muon_SA_RPCFit_innOffset                       = tap.muonSArpcFitInnOffset();
    muon_SA_RPCFit_innOffset                                            = &( tmp_muon_SA_RPCFit_innOffset );
    vector < float > tmp_muon_SA_RPCFit_midPhi                          = tap.muonSArpcFitMidPhi();
    muon_SA_RPCFit_midPhi                                               = &( tmp_muon_SA_RPCFit_midPhi );
    vector < float > tmp_muon_SA_RPCFit_midSlope                        = tap.muonSArpcFitMidSlope();
    muon_SA_RPCFit_midSlope                                             = &( tmp_muon_SA_RPCFit_midSlope );
    vector < float > tmp_muon_SA_RPCFit_midOffset                       = tap.muonSArpcFitMidOffset();
    muon_SA_RPCFit_midOffset                                            = &( tmp_muon_SA_RPCFit_midOffset );
    vector < float > tmp_muon_SA_RPCFit_outPhi                          = tap.muonSArpcFitOutPhi();
    muon_SA_RPCFit_outPhi                                               = &( tmp_muon_SA_RPCFit_outPhi );
    vector < float > tmp_muon_SA_RPCFit_outSlope                        = tap.muonSArpcFitOutSlope();
    muon_SA_RPCFit_outSlope                                             = &( tmp_muon_SA_RPCFit_outSlope );
    vector < float > tmp_muon_SA_RPCFit_outOffset                       = tap.muonSArpcFitOutOffset();
    muon_SA_RPCFit_outOffset                                            = &( tmp_muon_SA_RPCFit_outOffset );
    vector < vector < float > > tmp_muon_SA_roadAw                      = tap.muonSARoadAw();
    muon_SA_roadAw                                                      = &( tmp_muon_SA_roadAw );
    vector < vector < float > > tmp_muon_SA_roadBw                      = tap.muonSARoadBw();
    muon_SA_roadBw                                                      = &( tmp_muon_SA_roadBw );
    vector < vector < float > > tmp_muon_SA_zMin                        = tap.muonSAZMin();
    muon_SA_zMin                                                        = &( tmp_muon_SA_zMin );
    vector < vector < float > > tmp_muon_SA_zMax                        = tap.muonSAZMax();
    muon_SA_zMax                                                        = &( tmp_muon_SA_zMax );
    vector < vector < float > > tmp_muon_SA_rMin                        = tap.muonSARMin();
    muon_SA_rMin                                                        = &( tmp_muon_SA_rMin );
    vector < vector < float > > tmp_muon_SA_rMax                        = tap.muonSARMax();
    muon_SA_rMax                                                        = &( tmp_muon_SA_rMax );
    vector < vector < float > > tmp_muon_SA_etaMin                      = tap.muonSAEtaMin();
    muon_SA_etaMin                                                      = &( tmp_muon_SA_etaMin );
    vector < vector < float > > tmp_muon_SA_etaMax                      = tap.muonSAEtaMax();
    muon_SA_etaMax                                                      = &( tmp_muon_SA_etaMax );
    vector < vector < int > > tmp_muon_SA_MDTHit_isOutlier              = tap.muonSAmdtHitIsOutlier();
    muon_SA_MDTHit_isOutlier                                            = &( tmp_muon_SA_MDTHit_isOutlier );
    vector < vector < int > > tmp_muon_SA_MDTHit_chamber                = tap.muonSAmdtHitChamber();
    muon_SA_MDTHit_chamber                                              = &( tmp_muon_SA_MDTHit_chamber );
    vector < vector < float > > tmp_muon_SA_MDTHit_r                    = tap.muonSAmdtHitR();
    muon_SA_MDTHit_r                                                    = &( tmp_muon_SA_MDTHit_r );
    vector < vector < float > > tmp_muon_SA_MDTHit_z                    = tap.muonSAmdtHitZ();
    muon_SA_MDTHit_z                                                    = &( tmp_muon_SA_MDTHit_z );
    vector < vector < float > > tmp_muon_SA_MDTHit_phi                  = tap.muonSAmdtHitPhi();
    muon_SA_MDTHit_phi                                                  = &( tmp_muon_SA_MDTHit_phi );
    vector < vector < float > > tmp_muon_SA_MDTHit_residual             = tap.muonSAmdtHitResidual();
    muon_SA_MDTHit_residual                                             = &( tmp_muon_SA_MDTHit_residual );
    vector < vector < float > > tmp_muon_SA_MDTHit_space                = tap.muonSAmdtHitSpace();
    muon_SA_MDTHit_space                                                = &( tmp_muon_SA_MDTHit_space );
    vector < vector < float > > tmp_muon_SA_MDTHit_sigma                = tap.muonSAmdtHitSigma();
    muon_SA_MDTHit_sigma                                                = &( tmp_muon_SA_MDTHit_sigma );
    vector < int > tmp_muon_CB_HypoPass                                 = tap.muonCBHypoPass();
    muon_CB_HypoPass                                                    = &( tmp_muon_CB_HypoPass );
    vector < bool > tmp_muon_CB_OvRmPass                                = tap.muonCBOvRmPass();
    muon_CB_OvRmPass                                                    = &( tmp_muon_CB_OvRmPass );
    vector < double > tmp_muon_CB_pt                                    = tap.muonCBPt();
    muon_CB_pt                                                          = &( tmp_muon_CB_pt );
    vector < double > tmp_muon_CB_eta                                   = tap.muonCBEta();
    muon_CB_eta                                                         = &( tmp_muon_CB_eta );
    vector < double > tmp_muon_CB_phi                                   = tap.muonCBPhi();
    muon_CB_phi                                                         = &( tmp_muon_CB_phi );
    vector < int > tmp_muon_EF_pass                                     = tap.muonEFPass();
    muon_EF_pass                                                        = &( tmp_muon_EF_pass );
    vector < double > tmp_muon_EF_pt                                    = tap.muonEFPt();
    muon_EF_pt                                                          = &( tmp_muon_EF_pt );
    vector < double > tmp_muon_EF_eta                                   = tap.muonEFEta();
    muon_EF_eta                                                         = &( tmp_muon_EF_eta );
    vector < double > tmp_muon_EF_phi                                   = tap.muonEFPhi();
